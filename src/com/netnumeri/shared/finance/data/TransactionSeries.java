...
public class TransactionSeries implements FinConstants, Serializable {
    ...
    public TransactionSeries(String name) {
        this.name = name;
    ...
    // Transaction series normal constructor
    // If IsOwned flag is set, all added transactions are owned by
    // transaction series, i.e. they will be deleted together
    // with transaction series object
    public TransactionSeries(String Name, String Title, boolean IsOwner) {
        this.name = Name;
    }
    ...
    public TransactionSeries getSeries(Instrument instrument) {
        TransactionSeries series = new TransactionSeries(instrument.getName());
    ...
    public TransactionSeries getSeries(Date date) {
        TransactionSeries series = new TransactionSeries(getName());
        for (int i = 0; i < getNTransactions(); i++)
    ...
    public Object clone() {
        TransactionSeries ts = new TransactionSeries(getName());
