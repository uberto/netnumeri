...
/**
 * An immutable representation of Money. This class was created to reference sums of money. Due to rounding issues, the
 * use of <code>float</code> and to a lesser extent <code>double</code> are not recommended to represent money.
 * Therefore money should be represented by an index type, which would require conversion from the more natural
 * floating point type. This class was created to handle all this conversion work and provide a nice simple
 * abstraction.
 */
public class Money implements Cloneable, Comparable {

    // WARNING: Do not toDateTime doubles to long by:
    ...
    // This is private because if the webuser wants to newInstance an empty money
     ...
     * @throws MoneyFormatException if there was an error parsing the string.
     */
    public Money(String string)
            throws MoneyFormatException {

        try {
            // Try parsing currency enformat, e.g. $32.00
    ...
    /**
     * Return the value of this <code>Money</code> as a <code>long</code>. This conversion does not involve rounding as
     * it represents the number of cents in the money rather than the dollar amount.
     *
